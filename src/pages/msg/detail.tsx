...
const NewsDetail = () => {
    ...
    return <main className=" min-h-screen px-5">
        ...
        <>     <h2 className="py-4 text-xl font-semibold text-black">{data?.title}</h2>
            <div className=" text-right text-sm text-gray-500">{formatDate(data?.showTime)}</div>
            <div className=" text-sm text-gray-900 leading-6" dangerouslySetInnerHTML={{ __html: data?.content }}></div></>
